...
- **设计系统在 `web/src/index.css`**：字体（Fraunces 衬线标题 / Plus Jakarta Sans 正文 / Geist Mono 代码）+ OKLCH 颜色 token + 暗色变体 + grain 噪点 + hairline / ribbon-row / pulse-amber 等 utility。视觉走 **Claude 桌面端「暖米白纸感 + 陶土 coral」** 基调（accent 为 clay/coral ~hue 44；灰阶落在暖色 55–92 hue 带，避免冷中性）；暗色是 Claude 的暖炭灰（#262624 系，~hue 66–68）。改样式优先复用 token 和 utility，不要引入 hex / rgb 字面量。配色映射与改版说明见 `docs/spec/claude-desktop-redesign.md`。
