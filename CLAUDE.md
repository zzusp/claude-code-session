...
- **圆角按 surface 大小选 token**：按钮 / 输入 / nav / 下拉触发器 / 模态 footer CTA 用 `rounded-[var(--radius-control)]`（12px）；卡片 / callout 用 `--radius-card`（20px）；模态外壳 / 大面板用 `--radius-panel`。**不要**给小按钮套 `--radius-card`——20px 在 ≤40px 高的元素上会被 CSS 钳成半高变药丸。例外（保留自己形状）：药丸开关、纯图标按钮（≤36×36）、Breadcrumbs 紧密复合组件。完整说明见 `index.css` 注释块。
